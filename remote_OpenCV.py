# RC car remote program Python GUI
...
class RCCarController: # RC카 GuI 프로그램 클래스
    ...
    def create_layout(self): # GUI 전체 화면 배치
        ...
        self.camera_text = self.camera_canvas.create_text(
            260,
            195,
            text="카메라가 꺼져 있습니다.",
            fill="white",
        ...
        keyboard_label = ttk.Label(
            control_frame,
            text="키보드: W 전진 / S 후진 / A 좌회전 / D 우회전 / Space 정지",
            wraplength=260,
            justify="center",
            foreground="#555555"
        )
    ...
    def toggle_camera(self): # 카메라 켜기/끄기, OpenCV에 카메라 연결
        if not self.camera_on:
            print("카메라 연결 시도")

            # Windows에서는 MSMF가 잘 되는 경우가 많습니다.
            self.cap = cv2.VideoCapture(0, cv2.CAP_MSMF)

            if not self.cap.isOpened():
                print("0번 카메라 열기 실패")

                # MSMF가 실패하면 DirectShow로 다시 시도
                self.cap.release()
            ...
            # 이 줄이 반드시 있어야 화면이 갱신됩니다.
            self.show_camera()

        else:
            self.stop_camera()

    def show_camera(self): # OpenCV에서 프레임 읽어 실시간 출력
        if not self.camera_on:
            return
        ...
        if not success:
            ...
            self.camera_canvas.create_text(
                260,
                195,
                text="카메라 프레임을 읽을 수 없습니다.",
                fill="white",
                font=("맑은 고딕", 16)
            )

            # 잠시 후 다시 읽기 시도
            self.camera_job = self.window.after(
                100,
                self.show_camera
            )
            return
    ...
    def update_status(self, command): # 최근 실행 명령과 시간을 GUI에 표시
        ...
        self.time_label.config(
            text=f"최근 명령: {command} ({current_time})"
        )
...
"""
프로그램명: RC Car Control System

기능
1. RC카 연결/해제
2. 전진, 후진, 좌회전, 우회전, 정지
3. 속도 조절
4. 노트북 카메라 실시간 출력(OpenCV)
5. 화면 캠쳐
6. 키보드 (W, A, S, D Space) 조작
"""
